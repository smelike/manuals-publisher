...
describe FinderSchema do
  let(:sample_schema) { File.read(File.dirname(__FILE__) + "/../fixtures/sample_schema.json") }

  before do
    allow(File).to receive(:read).and_return(sample_schema)
  end

  let(:finder_schema) { FinderSchema.new(:sample_schema) }
